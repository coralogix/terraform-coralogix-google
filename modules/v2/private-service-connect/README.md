...
| network_self_link | Existing VPC network self link or id | string | - | yes |
| psc_subnet_name | Name of the PSC subnet | string | `"cx-psc-subnet"` | no |
| psc_subnet_cidr | CIDR block for the PSC subnet. Must be `/28`. | string | - | yes |
...
| dns_zone_name | Private Cloud DNS zone name, if created |
